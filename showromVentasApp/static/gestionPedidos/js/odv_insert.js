...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function submitForm() {
    ...
    const docEntry = document
      .getElementById("numero_orden")
      .getAttribute("data-docEntry");
    const docDate = fechaSolo;
    const fechaentrega = document.getElementById("fecha_entrega").value;
    const taxDate = fechaSolo;
    const rut = document
      .getElementById("inputCliente")
      .getAttribute("data-codigoSN");
    const cardCode = rut;
    const pgc = -1;
    const referencia = document.getElementById("referencia").value;
    const observaciones = document.getElementById("Observaciones-1").value;
    const spcInt = document
      .getElementById("vendedor_data")
      .getAttribute("data-codeVen");
    ...
    const ulfen = 1;
    const tipoDocElement = document.querySelector(
      "[name='tipoDocTributario']:checked"
    );
    ...
    productRows.forEach((row, index) => {
      ...
      const fechaentregaLineas = row.querySelector(
        "#fecha_entrega_lineas"
      ).value;
